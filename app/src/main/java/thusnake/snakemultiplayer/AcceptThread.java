...
public class AcceptThread extends Thread {
    ...
    public  AcceptThread() {
        ...
        try {
            ...
            tmp = OpenGLES20Activity.mBluetoothAdapter.listenUsingRfcommWithServiceRecord("host",uuid);
        } catch (IOException e) { }
        mmServerSocket = tmp;
    }
    ...
    private void manageConnectedSocket(BluetoothSocket socket) {
        ...
        for (index = 0; index < 4; index++) {
            if (OpenGLES20Activity.cnctdThreads[index] == null) {
                break;
            }
        }
        OpenGLES20Activity.cnctdThreads[index] = new ConnectedThread(socket);
        OpenGLES20Activity.cnctdThreads[index].start();
